class PeerBuilder {
  ...
  // adicionar o comportamenmto dos eventos de call também para quem ligar
  _preparePeerInstanceFunction(peerModule) {
    ...
    PeerCustomModule.prototype.call = function (id, stream) {
      ...
      // aqui acontece a magia, interceptamos o call e adicionamos todos os eventos da chamada para quem liga também
  ...
  build() {
    ...
    // ta na documentação do peerjs
    peer.on("error", this.onError);
    peer.on("call", this._prepareCallEvent.bind(this));

    // qnd conseguir estabilizar uma conexão
